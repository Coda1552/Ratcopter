...
public class RatcopterEntity extends Entity {
   private static final DataParameter<Integer> DATA_ID_HURT = EntityDataManager.defineId(RatcopterEntity.class, DataSerializers.INT);
   private static final DataParameter<Integer> DATA_ID_HURTDIR = EntityDataManager.defineId(RatcopterEntity.class, DataSerializers.INT);
   private static final DataParameter<Float> DATA_ID_DAMAGE = EntityDataManager.defineId(RatcopterEntity.class, DataSerializers.FLOAT);
   ...
   private RatcopterEntity.Status status;
   private RatcopterEntity.Status oldStatus;
   ...
   public RatcopterEntity(EntityType<? extends RatcopterEntity> p_i50129_1_, World p_i50129_2_) {
      super(p_i50129_1_, p_i50129_2_);
      this.blocksBuilding = true;
   }

   public RatcopterEntity(World p_i1705_1_, double p_i1705_2_, double p_i1705_4_, double p_i1705_6_) {
   ...
   protected void defineSynchedData() {
      ...
      this.entityData.define(DATA_ID_TYPE, RatcopterEntity.Type.OAK.ordinal());
   ...
   public void push(Entity p_70108_1_) {
      if (p_70108_1_ instanceof RatcopterEntity) {
   ...
   public void tick() {
      ...
      if (this.status != RatcopterEntity.Status.UNDER_WATER && this.status != RatcopterEntity.Status.UNDER_FLOWING_WATER) {
   ...
   private RatcopterEntity.Status getStatus() {
      RatcopterEntity.Status boatentity$status = this.isUnderwater();
      if (boatentity$status != null) {
         this.waterLevel = this.getBoundingBox().maxY;
         return boatentity$status;
      } else if (this.checkInWater()) {
         return RatcopterEntity.Status.IN_WATER;
      } else {
         float f = this.getGroundFriction();
         if (f > 0.0F) {
            this.landFriction = f;
            return RatcopterEntity.Status.ON_LAND;
         } else {
            return RatcopterEntity.Status.IN_AIR;
         }
      }
   }
   ...
   @Nullable
   private RatcopterEntity.Status isUnderwater() {
      AxisAlignedBB axisalignedbb = this.getBoundingBox();
            ...
            for(int i2 = i1; i2 < j1; ++i2) {
               ...
               if (fluidstate.is(FluidTags.WATER) && d0 < (double)((float)blockpos$mutable.getY() + fluidstate.getHeight(this.level, blockpos$mutable))) {
                  if (!fluidstate.isSource()) {
                     return RatcopterEntity.Status.UNDER_FLOWING_WATER;
                  }
      ...
      return flag ? RatcopterEntity.Status.UNDER_WATER : null;
   }
   ...
   private void floatBoat() {
      ...
      if (this.oldStatus == RatcopterEntity.Status.IN_AIR && this.status != RatcopterEntity.Status.IN_AIR && this.status != RatcopterEntity.Status.ON_LAND) {
         this.waterLevel = this.getY(1.0D);
         this.setPos(this.getX(), (double)(this.getWaterLevelAbove() - this.getBbHeight()) + 0.101D, this.getZ());
         this.setDeltaMovement(this.getDeltaMovement().multiply(1.0D, 0.0D, 1.0D));
         this.lastYd = 0.0D;
         this.status = RatcopterEntity.Status.IN_WATER;
      } else {
         if (this.status == RatcopterEntity.Status.IN_WATER) {
            d2 = (this.waterLevel - this.getY()) / (double)this.getBbHeight();
            this.invFriction = 0.9F;
         } else if (this.status == RatcopterEntity.Status.UNDER_FLOWING_WATER) {
            d1 = -7.0E-4D;
            this.invFriction = 0.9F;
         } else if (this.status == RatcopterEntity.Status.UNDER_WATER) {
            d2 = (double)0.01F;
            this.invFriction = 0.45F;
         } else if (this.status == RatcopterEntity.Status.IN_AIR) {
            this.invFriction = 0.9F;
         } else if (this.status == RatcopterEntity.Status.ON_LAND) {
   ...
   protected void readAdditionalSaveData(CompoundNBT p_70037_1_) {
      if (p_70037_1_.contains("Type", 8)) {
         this.setType(RatcopterEntity.Type.byName(p_70037_1_.getString("Type")));
      }
      ...
      if (!this.isPassenger()) {
         if (p_184231_3_) {
            if (this.fallDistance > 3.0F) {
               if (this.status != RatcopterEntity.Status.ON_LAND) {
   ...
   public void setType(RatcopterEntity.Type p_184458_1_) {
      this.entityData.set(DATA_ID_TYPE, p_184458_1_.ordinal());
   }

   public RatcopterEntity.Type getBoatType() {
      return RatcopterEntity.Type.byId(this.entityData.get(DATA_ID_TYPE));
   }
   ...
   public boolean isUnderWater() {
      return this.status == RatcopterEntity.Status.UNDER_WATER || this.status == RatcopterEntity.Status.UNDER_FLOWING_WATER;
   }
   ...
   public static enum Type {
      ...
      public static RatcopterEntity.Type byId(int p_184979_0_) {
         RatcopterEntity.Type[] aboatentity$type = values();
      ...
      public static RatcopterEntity.Type byName(String p_184981_0_) {
         RatcopterEntity.Type[] aboatentity$type = values();
